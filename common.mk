...
# Includes
INCLUDES  += -I. -I$(CUDA_INSTALL_PATH)/include -I$(COMMONDIR)/inc -I$(CUDA_INSTALL_PATH)/samples/common/inc \
	-I../ObjectTracker/include/ \
	-I../CaptureLib/include/ `pkg-config libavformat --cflags`\
	`pkg-config libavcodec --cflags` `pkg-config opencv --cflags`
...
# Libs
LIB := -L$(CUDA_INSTALL_PATH)/lib64 -L$(LIBDIR) -L$(COMMONDIR)/lib/$(OSLOWER) -lX11 -lboost_system \
	-L../CaptureLib/lib -lCapture -L../ObjectTracker/lib -lObjectTracker \
	`pkg-config libavformat --libs` `pkg-config libavcodec --libs` `pkg-config opencv --libs`
...
ifneq ($(STATIC_LIB),)
	...
	#fltk
	LIB += -lfltk2 -lXft -lfltk2_images -lXext -lXinerama -lXi -lX11
...
Tags:
	etags -o TAGS $(SRCDIR)/*.cpp
	etags -a -o TAGS $(SRCDIRUTILS)/*.cpp $(SRCDIRUTILS)/*.h
	ctags -o tags $(SRCDIRHOG)/*.cpp $(SRCDIRHOG)/*.h
	ctags -a -o tags $(SRCDIR)/*.cpp
	ctags -a -o tags $(SRCDIRUTILS)/*.cpp $(SRCDIRHOG)/*.h
